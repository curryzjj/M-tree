...
class MerkleTree {
     ...
     *```
     */
    constructor(leaves, hashAlgorithm = sha256_1.default, withSort, withBloomFilter, options = {}) {
        this.BloomFilter = new BloomFilter(10000000, 0.001)
        this.data = leaves;
        for (var i = 0; i < leaves.length; i++) {
            this.BloomFilter.add(leaves[i])
        }
        leaves = leaves.map(x => hashAlgorithm(x))
        this.withBloomFilter = withBloomFilter
        this.totalRequest = 0;
        this.fault_prediction = 0
        this.fault_rate = 0;
        this.hashAlgorithm = hashAlgorithm
        this.isBitcoinTree = !!options.isBitcoinTree;
        this.hashLeaves = !!options.hashLeaves;
        this.sortLeaves = !!withSort;
    ...
    _createHashes(nodes) {
         ...
         *```
         */
    insert(leaves, hashAlgorithm, options) {
        var preleaves = this.data;
        this.BloomFilter.add(leaves)
        preleaves.push(leaves)
        const tree = new MerkleTree(preleaves, hashAlgorithm, options)
    ...
    getProof(leaf, index) {
            this.totalRequest++;
            if (this.withBloomFilter && !this.BloomFilter.contain(leaf)) {
                return []
            }
            leaf = this.hashAlgorithm(leaf);
            ...
            if (index <= -1) {
                this.fault_prediction++;
                this.fault_rate = this.fault_prediction / this.totalRequest;
                return [];
            }
    ...
    verify(proof, targetNode, root) {
            targetNode = this.hashAlgorithm(targetNode)
